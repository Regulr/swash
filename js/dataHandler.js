...
let dataHandler = (function () {
  ...
  async function sendData(message, delay) {
    console.log('this is within sendData printing msg');
    console.log(message);
    if (delay) {
      ...
      //   streams[message.header.category].produceNewEvent(message); original Swash code for updating streams on Streamr
    }
  }
  ...
  async function prepareAndSend(message, module, delay, tabId) {
    ...
    if (module.context) {
      let bct_attrs = module.context.filter(function (ele, val) {
      ...
      let cct_attrs = module.context.filter(function (ele, val) {
        return ele.type === 'content' && ele.is_enabled;
      });

      if (cct_attrs.length > 0 && tabId) {
        var connectPort = browser.tabs.connect(tabId, {
  ...
  async function handle(message, tabId) {
    ...
    let collector = modules[message.header.module][
    ...
    profile.country
      ...
      : (country = await swashApiHelper.getUserCountry());
  ...
  function enforcePolicy(message, privacyData) {
    ...
    var ptr = JsonPointer;
    for (let d of schems) {
      let jpointers = JSONPath.JSONPath({
        path: d.jpath,
        resultType: 'pointer',
